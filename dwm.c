...
int is_substr(const char* substr, const char* str){
      ...
      while(str[i+len-1] != '\0'){
            ...
            buf[len]='\0';
            if(strcmp(buf, substr) == 0)return 1;
      }
      return 0;
}
void
...
maprequest(XEvent *e)
{
	...
	if (!wintoclient(ev->window))
		manage(ev->window, &wa);
            // set window with a method like this one
}
...
void shift_item(const Arg* arg){
      ...
      /*for(Monitor* m = mons; m->next != NULL; m = m->next){*/
      fprintf(fp, "arg: %s\n", ((char**)arg->v)[0]);
      char i = 0;
      for(Monitor* m = selmon; m != NULL; m = m->next){
            fprintf(fp, "monitor: #%i\n", ++i);
